...
const SkillDialog: React.FC<SkillDialogProps> = ({ isOpen, onOpenChange }) => {
  ...
  const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>, category:string, techName: string) => {
    ...
    setSelectedSkills((state) => {
        ...
        const newState = {
            ...state,
            [category]: isChecked && selectedTech
  ...
  const filteredTechnologies: TechnologiesType = Object.entries(technologies).reduce(
    (acc, [category, techs]) => ({
      ...acc,
          ...
          <div key={category}>
            ...
            {techs.map((tech: SkillType) => (
              <div key={tech.label} className="flex items-center mb-2">
                <input
                  type="checkbox"
                  checked={!!selectedSkills?.[category]?.find(skill => skill.label === tech.label) || false}
                  onChange={(event) => handleCheckboxChange(event, category, tech.label)}
                  className="mr-2"
                />
